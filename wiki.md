...
## Введение
TDLib (Three's Derma Library) - это библиотека для создания пользовательского интерфейса в Garry's Mod. Она предоставляет множество полезных классов для стилизации и анимации элементов интерфейса.

## Основное использование

Чтобы начать использовать TDLib, добавьте метод TDLib() к вашей панели:
...
Или используйте глобальную функцию:
...
### Background
Добавляет фоновый цвет панели с возможностью скругления углов.

```lua
panel:Background(Color(30, 30, 30), 5) -- цвет и радиус скругления
```

### Text
Добавляет текст к панели с настраиваемыми параметрами.

```lua
panel:Text("Привет мир!", "DermaLarge", Color(255, 255, 255), TEXT_ALIGN_CENTER)
```

### TextOutlined
Добавляет обведенный текст:

```lua
panel:TextOutlined("Привет мир!", "DermaLarge", Color(255, 255, 255), TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
```

### Hover эффекты

#### FadeHover
Создает эффект затемнения при наведении:

```lua
panel:FadeHover(Color(255, 255, 255, 30), 6) -- цвет и скорость анимации
```

#### BarHover
Добавляет анимированную полосу при наведении:

```lua
panel:BarHover(Color(255, 255, 255), 2, 6) -- цвет, высота, скорость
```

#### FillHover
Заполняет панель цветом при наведении в указанном направлении:

```lua
panel:FillHover(Color(255, 255, 255, 30), LEFT, 8) -- цвет, направление, скорость
```

### Градиенты

```lua
panel:Gradient(Color(255, 0, 0), TOP) -- цвет и направление
```
...
```lua
panel:CircleAvatar()
panel:SetPlayer(LocalPlayer(), 64) -- игрок и размер
```

### Анимации

#### CircleClick
Добавляет круговую анимацию при клике:

```lua
panel:CircleClick(Color(255, 255, 255, 50), 5)
```
...
```lua
panel:Blur(8) -- интенсивность размытия
```

#### SetOpenURL
Добавляет открытие URL при клике:
...
#### NetMessage
Отправляет сетевое сообщение при клике:

```lua
panel:NetMessage("MyNetworkString", function(pnl)
...
frame:Center()
frame:MakePopup()
frame:Background(Color(30, 30, 30), 5)
frame:Blur(5)
...
button:Background(Color(40, 40, 40), 4)
button:FadeHover(Color(60, 60, 60))
button:CircleClick()
```

## Примечания
- Все цвета могут быть настроены через параметр Color()
- Большинство методов поддерживают цепочку вызовов
- Скорость анимации может быть настроена через параметр speed
- Направления могут быть: LEFT, RIGHT, TOP, BOTTOM

Для более подробной информации о конкретных методах, смотрите исходный код:

```1:677:lua/libs/tdlib/tdlib.lua
/*---------------------------------------------------------------------------
	Three's Derma Lib
	Made by Threebow
